...
<body>
	<div class="wrapper">
		...
		<div class="products_list products_slider clear">
			<br>
			<h3 style="font-weight: normal;">${sessionScope.result.dataList[0].category.type }</h3>
			<br>
			<ul id="first-carousel"
				...
				<c:forEach items="${sessionScope.result.dataList}" var="product">
					<li><a
						href="${shop}/product_get.action?id=${product.id}&typeid=${product.category.id}"
						class="product_image"><img
							src="${shop}/image/product/${product.pic}"
		...
		<form id="proform">
			<input type="hidden" name="pageNum"> <input type="hidden"
				name="cid" value="${sessionScope.cid}">
		</form>
